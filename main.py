...
class CookieClicker(App):
    ...
    def build(self):
        ...
        self.factory_label = Label(text="Factories: 0", font_size=20, size_hint=(None, None), size=(150, 50), pos_hint={'right': 0.95, 'top': 0.65})
        self.layout.add_widget(self.factory_label)
        ...
        self.buy_factory_icon = Image(source='5095911.png', size_hint=(None, None), size=(50, 50), pos_hint={'right': 0.95, 'top': 0.25})
        self.buy_factory_icon.bind(on_touch_down=self.buy_factory)  
        self.layout.add_widget(self.buy_factory_icon)
        ...
        self.factory_count_label = Label(text="0", size_hint=(None, None), size=(50, 50), pos_hint={'right': 0.9, 'top': 0.25})
        self.layout.add_widget(self.factory_count_label)
    ...
    def buy_factory(self, instance, touch):
        if touch:
            if self.buy_factory_icon.collide_point(*touch.pos):
                if self.cookie_count >= 50:
                    self.cookie_count -= 50
                    self.cookie_label.text = "Cookies: {}".format(self.cookie_count)
                    self.factory_count += 1
                    self.factory_label.text = "Factories: {}".format(self.factory_count)
                    self.factory_count_label.text = str(self.factory_count)
